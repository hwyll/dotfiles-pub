...
if [ "$bold_status" = true ]; then
  status_left=$("$left_state" && get_tmux_option "@tmux-dotbar-status-left" "#[bg=$bg,fg=$fg_session]#{?client_prefix,, #S }#[bg=$bg,fg=$fg_prefix,nobold]#{?client_prefix, #S ,}#[bg=$bg,fg=${fg_session}]")
else
  status_left=$("$left_state" && get_tmux_option "@tmux-dotbar-status-left" "#[bg=$bg,fg=$fg_session]#{?client_prefix,, #S }#[bg=$bg,fg=$fg_prefix,bold]#{?client_prefix, #S ,}#[bg=$bg,fg=${fg_session}]")
fi
...
tmux set-option -g window-status-bell-style "bg=${bg},fg=${fg_prefix},bold"
